...
from small_business.reports.models import AccountBalance, BalanceSheetReport
...
def generate_balance_sheet(
	...
	for account in chart.accounts:
		if account.account_type == AccountType.ASSET:
			...
			if balance != 0:
				asset_accounts[account.name] = AccountBalance(
					name=account.name,
					balance=balance,
				)
	...
	for account in chart.accounts:
		if account.account_type == AccountType.LIABILITY:
			...
			if balance != 0:
				liability_accounts[account.name] = AccountBalance(
					name=account.name,
					balance=balance,
				)
	...
	for account in chart.accounts:
		if account.account_type == AccountType.EQUITY:
			...
			if balance != 0:
				equity_accounts[account.name] = AccountBalance(
					name=account.name,
					balance=balance,
				)
